...
//         ));
  ...
  // Menambah event ke Firebase
  Future<void> addEvent(String name, DateTime date, String time) async {
    try {
      await eventCollection.add({
        'name': name,
        'date': Timestamp.fromDate(date), // Menyimpan tanggal event
        'time': time, // Menyimpan waktu event
      });
    } catch (e) {
  ...
  // Mengambil event dan mengurutkannya berdasarkan tanggal dan waktu
  Future<Map<DateTime, List<String>>> getEventsAsMap() async {
    try {
      var snapshot = await eventCollection
          .orderBy('date') // Urutkan berdasarkan tanggal
          .orderBy('time') // Urutkan berdasarkan waktu
          .get();
      ...
      for (var doc in snapshot.docs) {
        ...
        eventMap[eventDate]?.add('$eventName at $eventTime');
      }
      return eventMap;
    } catch (e) {
